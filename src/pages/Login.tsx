...
function Layout() {
  ...
  useEffect(() => {
    const session = supabase.auth.session(); // Check if there's an active session
    if (session?.user) {
      // If user is authenticated, set user in store
    ...
    // Listen for auth state changes and update user session
